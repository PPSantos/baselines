...
@register("mlp")
def mlp(mlp_hiddens=[8], mlp_activation=tf.nn.relu, mlp_layer_norm=False):
    """
    ...
    mlp_hiddens: list                   List with hidden nodes.

    mlp_activation:                     Activation function (default: tf.nn.relu).

    mlp_layer_norm: bool                Layer normalization.
    ...
    def network_fn(X):
        ...
        for hidden in mlp_hiddens:
            h = tf.contrib.layers.fully_connected(h, num_outputs=hidden, activation_fn=None)
            if mlp_layer_norm:
